...
const initialState: PaymentState = {
  ...
  landlordPagination: { total: 0, page: 1, totalPages: 1 },
  isLoading: false,
  isProcessing: false,
  error: null,
};
...
const paymentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // fetch payments
      .addCase(fetchTenantPayments.pending, (state) => {
      ...
      .addCase(createDepositOrderThunk.pending, (state) => {
        state.isProcessing = true;
      })
      .addCase(createDepositOrderThunk.fulfilled, (state) => {
      ...
      // create rent order
      .addCase(createRentOrderThunk.pending, (state) => {
        state.isProcessing = true;
      })
      .addCase(createRentOrderThunk.fulfilled, (state) => {
        state.isProcessing = false;
      })
      ...
      .addCase(fetchLandlordPayments.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(fetchLandlordPayments.fulfilled, (state, action) => {
        state.isLoading = false;
        state.payments = action.payload.payments;
        state.landlordPagination = {
          total: action.payload.total,
          page: action.payload.page,
          totalPages: action.payload.totalPages,
        };
      })
      .addCase(fetchLandlordPayments.rejected, (state) => {
        state.isLoading = false;
      })
