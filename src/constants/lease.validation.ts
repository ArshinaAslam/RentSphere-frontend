...
export const createLeaseSchema = z
  .object({
    ...
    rentAmount: z
      .number({ error: "Rent amount must be a valid number" })
      .min(1, "Rent amount must be greater than 0"),

    securityDeposit: z
      .number({ error: "Security deposit must be a valid number" })
    ...
    gracePeriodDays: z.number().min(0).max(15).default(3),
    lateFee: z.number().min(0, "Late fee cannot be negative"),
  ...
  .refine((data) => data.securityDeposit <= data.rentAmount, {
    message: "Security deposit cannot be greater than rent amount",
