...
const senderRoleRef      = useRef<'tenant' | 'landlord'>('tenant');
 ...
 const cleanup = useCallback(() => {
  ...
  const createPeerConnection = useCallback((targetUserId: string, type: CallType) => {
...
const acceptCall = useCallback(async (acceptorRole: 'tenant' | 'landlord' = 'tenant') => {
...
setIsCameraOff(false);
    ...
    const pc = createPeerConnection(incomingCall.from, incomingCall.callType);
    stream.getTracks().forEach(t => pc.addTrack(t, stream));
...
const endCall = useCallback(() => {
  ...
  senderRoleRef.current     = (data as IncomingCallData & { callerRole?: 'tenant' | 'landlord' }).callerRole ?? 'landlord'
    });
