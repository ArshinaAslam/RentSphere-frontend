...
import {
  ...
  Clock, MapPin, Inbox, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
export default function EnquiriesPage() {
  ...
  useEffect(() => {
    void dispatch(fetchLandlordInquiries({
      page,
      limit:  LIMIT,
  ...
  const getTenant = (t: LandlordInquiry['tenantId']): TenantInfo | null =>
    typeof t === 'object' ? t as TenantInfo : null;
  ...
  />
        ...
        {!isLoadingLandlord && landlordInquiries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-1">No enquiries found</h3>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                      ...
                      <td className="px-5 py-4 max-w-xs">
                        <div className="space-y-1">
                          {inquiry.questions.map((q, i) => (
                            ...
                            >
                              <span className="text-emerald-500 font-bold flex-shrink-0">Q</span>
                              {q}
                            </div>
                          ))}
                          {inquiry.message && (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          {inquiry.status === 'unread' && (
                            <span className="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0" title="Unread" />
                          )}
                          <button
            ...
            <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100">
              <p className="text-xs text-slate-400">
                Showing {Math.min((page - 1) * LIMIT + 1, landlordInquiriesTotal)}–{Math.min(page * LIMIT, landlordInquiriesTotal)} of {landlordInquiriesTotal} enquiries
              </p>
