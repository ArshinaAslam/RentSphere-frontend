...
} from "@/features/landlordVisit/landlordVisitThunk";
import { setLeasePreFill } from "@/features/lease/leaseSlice";
...
export default function VisitRequestsPage() {
  ...
  const { visits, isLoading, updatingId, total, totalPages } = useAppSelector(
    (s: RootState) => s.landlordVisit,
  );
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-200">
            ...
            <p className="text-slate-400 text-sm">
              {activeTab === "all"
                ? "No tenants have scheduled visits yet"
                : `No ${activeTab} visits`}
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((visit) => {
                  const tenant =
                    typeof visit.tenantId === "object" ? visit.tenantId : null;
                  const property =
                    typeof visit.propertyId === "object"
                      ? visit.propertyId
                      : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${statusBadge(visit.status)}`}
                        >
                          {visit.status}
                        </span>
                      </td>
                        ...
                        <div className="flex flex-col gap-2">
                              ...
                              onClick={() => {
                                ...
                                dispatch(
                                  setLeasePreFill({
                                    ...
                                    rentAmount: String(property?.price ?? ""),
                                ...
                                router.push("/landlord/leases/create");
                              }}
                              className="flex items-center gap-1.5 px-3 py-2 border border-purple-200 text-purple-600 hover:bg-purple-50 text-xs font-semibold rounded-xl transition"
                            >
                          ...
                          {visit.status === "confirmed" && (
                            <button
                              ...
                              disabled={isUpdating}
                              className="flex items-center gap-1.5 px-3 py-2 border border-emerald-200 text-emerald-600 hover:bg-emerald-50 text-xs font-semibold rounded-xl transition disabled:opacity-50"
                            >
                              {isUpdating ? (
                          ...
                          {canCancel &&
                            (isConfirming ? (
                              <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl p-2.5 w-48">
                                ...
                                <div className="flex gap-1.5">
                                  <button
                                    ...
                                    disabled={isUpdating}
                                    className="px-2.5 py-1 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-lg transition"
                                  >
                                    {isUpdating ? (
