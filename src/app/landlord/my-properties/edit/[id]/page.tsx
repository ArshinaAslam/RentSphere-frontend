  ...
  useEffect(() => {
    if (selectedProperty) {
      ...
      if (typeof rawAmenities === "string") {
        try {
          parsedAmenities = JSON.parse(rawAmenities)as string[];;
        } catch {
          parsedAmenities = [];
        }
      } else if (Array.isArray(rawAmenities)) {
        if (
          rawAmenities.length === 1 &&
          typeof rawAmenities[0] === "string" &&
          rawAmenities[0].startsWith("[")
        ) {
          try {
            parsedAmenities = JSON.parse(rawAmenities[0])as string[];;
          } catch {
            parsedAmenities = [];
          }
        } else {
          parsedAmenities = rawAmenities.filter(
  ...
  const handleSave = handleSubmit(async (data: PropertyFormValues) => {
    ...
    try {
      ...
      const result = await dispatch(
        updateLandlordProperty({ id: propertyId, formData: fd }),
      );
      if (updateLandlordProperty.fulfilled.match(result)) {
        toast.success("Property updated successfully!");
        void dispatch(fetchLandlordProperties({ page: 1, search: "" }));
          ...
          <div className="space-y-5">
                    ...
                    <Field label="Property Type">
                      <Controller
                        ...
                        render={({ field }) => (
                          <select
                            {...field}
                            className={`${sClass} ${errors.type ? "border-red-300 ring-red-500/20" : ""}`}
                          >
                    ...
                    <Field label="BHK Configuration">
                      <Controller
                        ...
                        render={({ field }) => (
                          <select
                            {...field}
                            className={`${sClass} ${errors.bhk ? "border-red-300 ring-red-500/20" : ""}`}
                          >
                  ...
                  <Field label="Furnishing Status">
                    <Controller
                      ...
                      render={({ field }) => (
                        <select
                          {...field}
                          className={`${sClass} ${errors.furnishing ? "border-red-300 ring-red-500/20" : ""}`}
                        >
            ...
            {activeSection === "location" && (
              <Card title="Location Details" icon={<MapPin size={16} />}>
                ...
                <NavButtons
                  prev={() => setActiveSection("basic")}
                  next={() => setActiveSection("pricing")}
                />
              </Card>
            )}
            ...
            {activeSection === "pricing" && (
              ...
              >
                  ...
                  <Field label="Status">
                    <Controller
                      ...
                      render={({ field }) => (
                        <select
                          {...field}
                          className={`${sClass} ${errors.status ? "border-red-300 ring-red-500/20" : ""}`}
                        >
                          {statusOptions.map((s) => (
                ...
                <NavButtons
                  prev={() => setActiveSection("location")}
                  next={() => setActiveSection("details")}
                />
              </Card>
            )}
            ...
            {activeSection === "details" && (
              <Card title="Property Details" icon={<Maximize2 size={16} />}>
                ...
                <NavButtons
                  prev={() => setActiveSection("pricing")}
                  next={() => setActiveSection("amenities")}
                />
              </Card>
            )}
            ...
            {activeSection === "amenities" && (
              <Card title="Amenities" icon={<Sparkles size={16} />}>
                  ...
                  <div className="mt-5 pt-4 border-t border-slate-100">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedAmenities.map((a) => {
                        const amenity = activeAmenities.find(
                          (item) => item.label === a,
                        );
                        return (
                          <span
                            key={a}
                            className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 bg-emerald-50 border border-emerald-300 text-emerald-700 text-xs font-medium rounded-full"
                          >
                ...
                <NavButtons
                  prev={() => setActiveSection("details")}
                  next={() => setActiveSection("images")}
                />
              </Card>
            )}
            ...
            {activeSection === "images" && (
              <Card title="Property Images" icon={<Upload size={16} />}>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                      {existingImages.map((url, i) => (
                        <div
                          key={`existing-${i}`}
                          className="group relative rounded-2xl overflow-hidden aspect-video shadow-md border-2 border-emerald-100 hover:border-red-300 hover:shadow-2xl transition-all duration-300 cursor-pointer"
                        >
                          <img
                            src={url}
                            alt={`Image ${i + 1}`}
                          ...
                          <div className="absolute bottom-2 left-2 bg-emerald-600/90 text-white text-xs font-bold px-3 py-1 rounded-full">
                            Saved
                          </div>
                        </div>
                      ))}
                      ...
                      {newImages.map((img, i) => (
                        <div
                          key={`new-${i}`}
                          className="group relative rounded-2xl overflow-hidden aspect-video shadow-md border-2 border-blue-100 hover:border-red-300 hover:shadow-2xl transition-all duration-300 cursor-pointer"
                        >
                          <img
                            src={img.url}
                            alt={`New image ${i + 1}`}
                          ...
                          <div className="absolute bottom-2 left-2 bg-blue-500/90 text-white text-xs font-bold px-3 py-1 rounded-full">
                            New
                          </div>
                        </div>
                      ))}
                ...
                <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 hover:bg-emerald-50 hover:border-emerald-300 transition cursor-pointer group">
                  <Upload
                    size={20}
                    className="text-slate-400 group-hover:text-emerald-500 mb-1.5"
                  />
                ...
                <div className="mt-4 flex items-center justify-between">
                  <span
                    className={`text-sm font-medium ${existingImages.length + newImages.length >= 3 ? "text-emerald-600" : "text-slate-500"}`}
                  >
...
// ── Helpers  ─────────────────
const iClass =
  "h-11 border-slate-200 focus-visible:ring-emerald-500 focus-visible:border-emerald-500 rounded-xl";
const sClass =
  "w-full h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500";
