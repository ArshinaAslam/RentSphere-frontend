...
const STATUS_CONFIG = {
  ...
  expired: {
    ...
    icon: Clock,
    desc: "Lease has ended",
  },
  terminated: {
    label: "Terminated",
    color: "bg-red-100 text-red-700",
    icon: XCircle,
    desc: "Terminated early",
  },
};
...
export default function TenantLeasePage() {
  ...
  const pendingLeases = leases.filter((l: Lease) =>
    ["sent", "viewed"].includes(l.status),
  );
  const otherLeases = leases.filter(
    (l: Lease) => !["active", "sent", "viewed"].includes(l.status),
  );
          ...
          {leases.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-2xl border border-slate-100 shadow-sm">
              ...
              <p className="text-slate-400 text-sm">
                Your landlord will send you a lease agreement when ready
              </p>
            </div>
          ) : (
              ...
              {activeLease && (
                <div>
                  ...
                  <LeaseCard
                    lease={activeLease}
                    onClick={() =>
                      router.push(`/tenant/my-lease/${activeLease._id}`)
                    }
                  ...
                  <div className="space-y-3">
                    {pendingLeases.map((lease: Lease) => (
                      <LeaseCard
                        ...
                        lease={lease}
                        onClick={() =>
                          router.push(`/tenant/my-lease/${lease._id}`)
                        }
                        formatDate={formatDate}
                        formatCurrency={formatCurrency}
                      />
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        ...
        >
          <FileText
            className={`w-5 h-5 ${lease.status === "active" ? "text-emerald-600" : "text-slate-400"}`}
          />
        </div>
      ...
      {lease.status === "viewed" && (
        <div className="mt-3 pt-3 border-t border-blue-100 flex items-center gap-2">
          ...
          <p className="text-xs text-blue-600 font-medium">
            You have reviewed this — ready to sign
          </p>
        </div>
      )}
      {lease.status === "signed" && lease.tenantSignature && (
        <div className="mt-3 pt-3 border-t border-emerald-100 flex items-center gap-2">
          ...
          <p className="text-xs text-emerald-600 font-medium">
            Signed by {lease.tenantSignature.name} on{" "}
