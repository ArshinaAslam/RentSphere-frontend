...
export default function PropertyDetailPage() {
 const { id: propertyId } = useParams<{ id: string }>();
  ...
  useEffect(() => {
    if (userData?.id) {
      void dispatch(
        fetchWishlist({ tenantId: userData.id, page: 1, limit: 100 }),
      );
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div
        className={showInquiry ? "blur-sm pointer-events-none select-none" : ""}
      >
        <Navbar />
        ...
        <div className="max-w-6xl mx-auto px-6 pt-24 pb-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div className="lg:col-span-2">
              <div
                className="relative h-[400px] bg-slate-200 rounded-2xl overflow-hidden cursor-none"
                ...
                onMouseMove={(e) => {
                  ...
                  const bgX =
                    ((x - lensSize / 2) / (rect.width - lensSize)) * 100;
                  const bgY =
                    ((y - lensSize / 2) / (rect.height - lensSize)) * 100;

                  setBgPos({
                    x: Math.min(100, Math.max(0, bgX)),
                    y: Math.min(100, Math.max(0, bgY)),
                  });
                }}
              >
                ...
                {showLens && images.length > 0 && !imgErr[activeImg] && (
                  <div
                    className="absolute rounded-full pointer-events-none border-2 border-white shadow-2xl"
                    style={{
                      width: 120,
                      height: 120,
                      left: lensPos.x,
            ...
            <div className="pb-7 border-b border-slate-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  ...
                  {
                    label: "Area",
                    value: property.area ? `${property.area} sqft` : null,
                  },
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-8">
                  {amenitiesList.map((a: string) => {
                    ...
                    return (
                      <div
                        key={a}
                        className="flex items-center gap-3 text-sm text-emrald-700 py-2 border-b border-slate-100"
                      >
                        <Icon
                          size={16}
      ...
      {lightbox && images.length > 0 && (
        ...
        >
          ...
          <div
            className="absolute top-5 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-3 py-1.5 z-10"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setZoomLevel((z) => Math.max(1, z - 0.25))}
              className="w-7 h-7 rounded-full bg-white/20 hover:bg-white/30 text-white font-bold text-lg flex items-center justify-center transition"
            >
              −
            </button>
            ...
            <button
              ...
              className="text-white/70 hover:text-white text-xs font-medium transition px-1"
            >
              Reset
            </button>
          </div>
          ...
          <button
            className="absolute left-5 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition z-10"
            onClick={(e) => {
              e.stopPropagation();
          ...
          <div
            className="overflow-hidden max-h-[82vh] max-w-[82vw]"
            onClick={(e) => e.stopPropagation()}
          >
            <img
              src={images[activeImg]}
              ...
              className="max-h-[82vh] max-w-[82vw] object-contain rounded-xl block"
            />
          </div>

          <button
            className="absolute right-5 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition z-10"
            onClick={(e) => {
              e.stopPropagation();
              setActiveImg((i) => (i + 1) % images.length);
            }}
          ...
          <p className="absolute bottom-16 left-1/2 -translate-x-1/2 text-white/60 text-sm">
            {activeImg + 1} / {images.length}
          </p>
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          <div className="col-span-2">
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-20 h-14 bg-slate-100 rounded-xl" />
              ))}
            </div>
          </div>
