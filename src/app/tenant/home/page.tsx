...
export default function TenantHomePage() {
  ...
  const displayed = useMemo(() => {
    ...
    if (selectedStatus) r = r.filter((p) => p.status === selectedStatus);

    if (minBed) r = r.filter((p) => Number(p.bedrooms) >= Number(minBed));

    if (minBath) r = r.filter((p) => Number(p.bathrooms) >= Number(minBath));
  ...
  if (sortBy !== "newest")
    activeFilters.push({
      label:
        SORT_OPTIONS.find((opt) => opt.value === sortBy)?.label || "Sorted",
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <header className="bg-emerald-700 mt-16 relative">
        ...
        <div className="max-w-7xl mx-auto px-8 pt-8 pb-8 flex flex-col items-center text-center">
          ...
          <p className="text-white/70 text-lg mb-8 max-w-lg">
            Discover thousands of residential properties from apartments to
            villas, all in one place.
          </p>
      ...
      <div className="w-full px-12 py-10 mt-6 flex gap-8">
        <aside className="w-64 bg-white rounded-2xl shadow-sm border border-slate-200 sticky top-24 flex flex-col">
            ...
            <div className="flex items-center justify-between mt-10 pt-6 border-t border-slate-200">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={page === 1}
                  className="w-9 h-9 flex items-center justify-center rounded-xl border border-slate-200 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition"
                >
